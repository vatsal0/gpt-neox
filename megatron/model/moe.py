        ...
        if self.log_sims: 
          with torch.no_grad():
            ...
            for k in range(3):
              ...
              local_stats[f'avg_sim_{k}'] = avg_sim
              local_stats[f'pct_sim_{k}'] = pct_above
            ...
            local_stats['exp_avg_sim'] = exp_avg_sim / expert_output.shape[1]

            global_stats = {}
            for key in local_stats.keys():
              global_stats[key] = torch.distributed.all_reduce(
                local_stats[key],
